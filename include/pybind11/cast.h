...
instance::get_value_and_holder(const type_info *find_type /*= nullptr default in common.h*/,
    ...
    pybind11_fail("pybind11::detail::instance::get_value_and_holder: `"
                  ...
                  + "' is not a pybind11 base of the given `" + std::string(Py_TYPE(this)->tp_name)
                  + "' instance");
#endif
}
...
public:
    bool load(handle src, bool convert) {
        ...
        if (std::is_floating_point<T>::value) {
            ...
            py_value = sizeof(T) <= sizeof(long) ? (py_type) PyLong_AsLong(src.ptr())
...
private:
    static handle decode_utfN(const char *buffer, ssize_t nbytes) {
        ...
        return PyUnicode_Decode(
            buffer, nbytes, UTF_N == 8 ? "utf-8" : UTF_N == 16 ? "utf-16" : "utf-32", nullptr);
#endif
    }
...
using overload_caster_t = conditional_t<cast_is_temporary_value_reference<ret_type>::value,
...
tuple make_tuple(Args &&... args_) {
    ...
    for (size_t i = 0; i < args.size(); i++) {
        if (!args[i]) {
#if defined(NDEBUG)
            throw cast_error("make_tuple(): unable to convert arguments to Python object (compile "
                             "in debug mode for details)");
#else
            std::array<std::string, size> argtypes{{type_id<Args>()...}};
            throw cast_error("make_tuple(): unable to convert argument of type '" + argtypes[i]
                             + "' to Python object");
#endif
        }
    }
...
private:
    ...
    [[noreturn]] static void argument_cast_error(std::string name, std::string type) {
        throw cast_error("Unable to convert call argument '" + name + "' of type '" + type
                         + "' to Python object");
    }
